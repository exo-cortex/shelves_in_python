...
class Shelf:
	...
	def __str__(self):
		...
		for i in range(self.levels):
			output += "level {}: height = {}, widths = {}".format(i, self.level_heights[i], self.compartment_widths[i])
			if not i == self.levels - 1:
				output += "\n"
		return output

	def add_level(self, height, widths: list):
		# if sum(self.level_heights + height) + (1 + len(self.level_heights + height)) <= self.fullheight:
